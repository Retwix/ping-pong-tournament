...
export async function createPlayer(
	...
	const { data, error } = await supabase
		.from("players")
		.insert({ name, team, avatar_url: avatarUrl ?? null })
		.select()
		.single();
	if (error) throw error;
	return data as Player;
...
/** Update a player's name, team, and/or photo. */
...
/**
 * Upload a profile picture to the public `avatars` Storage bucket and return its
 * public URL. The file is stored under a random name so re-uploads never clash.
 * Requires avatars-migration.sql (bucket + policies) to have been run.
 */
...
export async function createTournament(
	...
	const effectiveFormat: TournamentFormat =
		kind === "game" ? "round_robin" : format;
	...
	const ordered =
		effectiveFormat === "double_elim" ? players.slice() : shuffle(players);
